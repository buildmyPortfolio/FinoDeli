...
function applyTheme(theme) {
  ...
  localStorage.setItem('fino-theme', theme);
}

// Restore saved preference or detect system preference on first visit
(function initTheme() {
  const saved = localStorage.getItem('fino-theme');
  if (saved) {
    applyTheme(saved);
...
function openItemModal(card) {
  ...
  if (descText) {
    ...
    iModalDesc.textContent = 'A carefully crafted dish prepared with the finest ingredients, served with Fino Deli\'s signature rustic touch.';
...
(function () {
  ...
  if (localStorage.getItem('fino-specials-dismissed') === '1') {
    bar.classList.add('dismissed');
  }
  closeBtn.addEventListener('click', () => {
    bar.classList.add('dismissed');
    localStorage.setItem('fino-specials-dismissed', '1');
  });
})();
...
function handleReservation(event) {
  ...
  let msg = `Hi Fino Deli! 🍽️ I'd like to make a reservation.\n\n`;
  msg    += `👤 Name: ${name}\n`;
  msg    += `📞 Phone: ${phone}\n`;
  msg    += `📅 Date: ${date}\n`;
  msg    += `🕐 Time: ${time}\n`;
  ...
  if (waLink) waLink.href = `https://wa.me/639063884248?text=${encodeURIComponent(msg)}`;
